package sollecitom.services.modulith_example.modules.payment_command_endpoint.adapters.driving.http.endpoint.serde.argument
...
private object ArgumentsJsonSerde : JsonSerde.SchemaAware<SendPayment.Arguments> {

    private const val SCHEMA_LOCATION = "modulith-example/payment-command-endpoint/arguments/Arguments.json"
    override val schema by lazy { jsonSchemaAt(SCHEMA_LOCATION) }

    override fun serialize(value: SendPayment.Arguments) = JSONObject().apply {
    ...
    override fun deserialize(value: JSONObject) = with(value) {
        ...
        SendPayment.Arguments(sourceAccount = sourceAccount, targetAccount = targetAccount, amount = amount)
    }
...
val SendPayment.Arguments.Companion.jsonSerde: JsonSerde.SchemaAware<SendPayment.Arguments> get() = ArgumentsJsonSerde